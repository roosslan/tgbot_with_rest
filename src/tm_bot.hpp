...
namespace manapi {
    ...
    public:
        ...
        manapi::future<manapi::error::status_or<manapi::json>>  send_message (int64_t user_id, manapi::json data, bool disable_link_preview = false, manapi::async::cancellation_action cancellation = nullptr);
        /**
...
err:
    ...
    /* rasa 12.9.25 */
    inline manapi::future<manapi::error::status_or<manapi::json>>  maxbot::send_message(int64_t user_id, manapi::json data, bool disable_link_preview, manapi::async::cancellation_action cancellation) {
        std::string const purl = std::format("/messages?user_id={}&disable_link_preview={}&", user_id, disable_link_preview);
        MANAPI_MAXBOT_STATUS_WRAP(fetch_post_bot_simple_(purl, data.dump(), std::move(cancellation)));
    }
    ...
    inline std::string maxbot::generate_url_(const std::shared_ptr<maxbot::data_t> &data, std::string_view path) {
        if (!path.empty() && (path.back() == '&'||path.back()=='?'))
            ...
            return std::format("https://botapi.tamtam.chat{}access_token={}", path, data->token);
